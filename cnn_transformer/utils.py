...
def checkpoint(net, history, epoch, optimizer, count_iter, args):
    ...
    torch.save(net.state_dict(), '{}/package_{}'.format(args.save_ckpt, suffix_latest))

    if epoch % args.save_freq == 0:
        torch.save(net.state_dict(), '{}/package_{}.pth'.format(args.save_ckpt, epoch))
...
def get_linear_schedule_with_warmup(optimizer, num_warmup_steps, num_training_steps, last_epoch=-1):
    """
    https://github.com/huggingface/transformers/blob/v4.26.0/src/transformers/optimization.py
    ...
    def lr_lambda(current_step: int):
        ...
        return max(
            # 0.0, float(num_training_steps - 0.9 * current_step - 0.1 * num_warmup_steps) / float(max(1, num_training_steps - num_warmup_steps))
